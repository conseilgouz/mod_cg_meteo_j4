<?php
/**
* Simple meteo module from https://openweathermap.org
* Version			: 2.0.3
...
foreach ($previsions as $prevision) {
	...
	$path_icon = JURI::base().'media/mod_cg_meteo/icons/open/'.$icon;
    ...
    if ($i == 0) { // aujourd'hui
		...
		if ($actuelle['direction'])  $label .= '&nbsp;'.JText::_( 'METEOFO_DIRECTION_'.(ceil(((int)$actuelle['direction']-5.62)/11.25)+1) );
		$label .= '</p>';
		$time12_24_leve = substr($actuelle['leve'], -8,5);
		$heures_leve = (int)substr($actuelle['leve'],-8 ,2);
		...
		$time12_24_couche = substr($actuelle['couche'], -8,5);
		$heures_couche = (int)substr($actuelle['couche'], -8 ,2);
